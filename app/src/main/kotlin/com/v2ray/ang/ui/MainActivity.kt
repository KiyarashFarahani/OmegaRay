...
import com.v2ray.ang.network.ConfigsObject
                    ...
                    if (serverList.isNotEmpty()){
                        ...
                        val delay =
                            MmkvManager.decodeServerAffiliationInfo(serverList[0])?.testDelayMillis
                                ?: 0L
                        Log.v("Kiyarash Delay", delay.toString())

                        if (delay != 0L && delay.toInt() != -1){
                            start()
                            hideCircle()
                        } else
                            connectToNewServer()
...
//                tv_test_state.text = getString(R.string.connection_test_fail)
    ...
    private suspend fun connectToNewServer() {
        val responses = ConfigsObject.retrofitService.getJsonData().lines()
        MmkvManager.removeAllServer()

        for (response in responses) {
            val res = AngConfigManager.importBatchConfig(response, "", true)
            if (res == 0)
                continue
            ...
            val delay =
                MmkvManager.decodeServerAffiliationInfo(serverList[0])?.testDelayMillis
                    ?: 0L
            Log.v("Kiyarash Config", response)
            Log.v("Kiyarash Delay", delay.toString())


            if (delay == 0L || delay.toInt() == -1)
                MmkvManager.removeServer(mainViewModel.serversCache[0].guid)
            else {
                start()
                hideCircle()
                break
    ...
    private fun observeJsonData() {
        mainViewModel.jsonResponse?.observe(this) {
            for (i in 0..20)
                importBatchConfig(it[i].config)
            mainViewModel.testAllRealPing()
        ...
        mainViewModel.isRunning.observe(this) { isRunning ->
            ...
            if (isRunning) {
                if (!Utils.getDarkModeStatus(this)) {
                    binding.fab.setImageResource(R.drawable.omega_foreground)
                }
...
//        }
    ...
    fun importClipboard()
        ...
        try {
            ...
            mainViewModel.getConfigsData()
            val clipboard = mainViewModel.jsonResponse

            Log.v("KIYARASH", clipboard.toString())
